...
def register():
    ...
    while askRegister == False:
        ...
        userExists = False
        for user in dbList:
            if (user["phone number"] == phoneNumber):
                    ...
                    if destinationID == j["id"]:
                        if destinationID != i["id"]:
                            transferAmount = input(
                                "enter the amount you want to transfer to " + j["name"] + " only in EGP: ")
                            ...
                            if transferAmount <= i["currentBalance"]:
                                ...
                                with open("data.json", "w") as file:
                                    json.dump(dbList, file, indent=2)
                                print("you transfered " + str(transferAmount) + " EGP to " + j[
                                    "name"] + " successfully!! ")
                            ...
                            print("HAHAHA…Nice try, but you can’t transfer money to yourself, " + i[
                                "name"] + " !!!!!!!!!!!!")
                            print("---------------------------------------------------------")
...
def startMenu():
    ...
    print("-------------------------------------------------")
def deleteAcc():
    global dbList, online, enter, login
    askDelete = input("ARE YOU SURE, you want to delete your account? [1] yes [2] no")
    if askDelete == "1":
        checkPassword = input("please enter your password: ")
        if checkPassword == loginPass:
            for i in dbList:
                if i["id"] == loginID:
                    dbList.remove(i)
...
while enter == False :
    ...
    if who == "1": ##login
                    ...
                    if loginID == i["id"]:
                        if loginPass == i["password"]:
                            ...
                            while online == True :
                                ...
                                if theOperation.isdigit() and (theOperation == "0" or theOperation == "1" or theOperation == "2" or theOperation == "3" or theOperation == "4" or theOperation == "5" or theOperation=="6") :
        ...
        machinePassword = input("Please enter the password of the machine: ")
        if machinePassword == "manger1":
            print("***************SHUTTING DOWN THE MACHINE***************")
            enter = True
        else:
            print("wrong password")
    else:
        print("please, only [1] or [2] or [3] ")
